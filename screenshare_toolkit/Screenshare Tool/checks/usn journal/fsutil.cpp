...
static void buildHashTable(FILE* file, FileData*** hashTable, int* tableSize) {
        ...
        if (strstr(line, "Rename:") != nullptr) {
            ...
            *hashTable = tempTable;
            ...
            if (positions[5] != nullptr) {
                ...
                // positions[2] can be null when the loop breaks.
    ...
    for (int i = 0; i < tableSize1; i++) {
        for (int j = 0; j < tableSize2; j++) {
            if (strcmp(hashTable1[i]->fileName, hashTable2[j]->fileName) == 0 &&
                ...
                if (!alreadyPrinted) {
                    // Use a temporary pointer to avoid potential memory leaks
                    FileData* tempLines = reinterpret_cast<FileData*>(realloc(printedLines, (static_cast<unsigned long long>(numPrintedLines) + 1) * sizeof(FileData)));
...
static void processLogFileComparison() {
    ...
    buildHashTable(file1, &hashTable1, &tableSize1);
    buildHashTable(file2, &hashTable2, &tableSize2);
...
static void RunCommand(TCHAR driveLetter, bool scanMacroTraces) {
    ...
    if (scanMacroTraces) {
        ...
        _stprintf_s(command, _T("fsutil usn readjournal %c: csv | findstr /i /c:0x00001000 >> .\\old_renamed_files.txt"), driveLetter);
        system(command);
        ...
        _tprintf(_T("[NTFS Scanner] Skipping macro trace detection based on user input.\n"));

        // Execute only the commands related to file rename modifications and special characters
        _tprintf(_T("[NTFS Scanner] Detecting file rename modifications in the entire NTFS drive...\n"));
...
void USNJournal() {
    ...
    bool scanMacroTraces = (userInput == 'y' || userInput == 'Y');
        ...
        if (_tcslen(pDrive) == 3 && pDrive[1] == ':' && pDrive[2] == '\\') {
            ...
            if (GetVolumeInformation(pDrive, nullptr, 0, nullptr, nullptr, nullptr, szFileSystem, MAX_PATH)) {
                if (_tcsicmp(szFileSystem, _T("NTFS")) == 0) {
                    RunCommand(pDrive[0], scanMacroTraces);
                }
                else {
    ...
    // Replace backslashes with forward slashes in the string representation of the path
    std::string pathString = currentPath.string();
    std::replace(pathString.begin(), pathString.end(), '\\', '/');

    // Print the current working directory with single forward slashes
    std::cout << "File modifications were exported to: " << pathString << std::endl;
