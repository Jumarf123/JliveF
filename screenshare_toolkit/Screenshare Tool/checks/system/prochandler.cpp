...
static bool IsProcessRestartedSinceLastLogon(DWORD processId, const SYSTEMTIME& lastLogonTime) {
    ...
    if (GetProcessCreationTime(processId, creationTime)) {
        ...
        // Calculate threshold time (last boot time + 120 seconds)
        FILETIME thresholdFileTime{};
        ULONGLONG thresholdIntervals = lastLogonFileTime.dwHighDateTime;
        thresholdIntervals <<= 32;
        thresholdIntervals |= lastLogonFileTime.dwLowDateTime;
        thresholdIntervals += static_cast<unsigned long long>(120) * 10000000;  // 120 seconds in 100-nanosecond intervals
        thresholdFileTime.dwLowDateTime = static_cast<DWORD>(thresholdIntervals);
        thresholdFileTime.dwHighDateTime = static_cast<DWORD>(thresholdIntervals >> 32);

        // Print last logon time
        SYSTEMTIME localLastLogonSystemTime;
        FileTimeToSystemTime(&lastLogonFileTime, &localLastLogonSystemTime);

        // Process creation time
        FILETIME localCreationTime;
        if (FileTimeToLocalFileTime(&creationTime, &localCreationTime)) {
            SYSTEMTIME creationSystemTime;
            FileTimeToSystemTime(&localCreationTime, &creationSystemTime);

            // Check if the creation time is after the threshold time
            if (CompareFileTime(&localCreationTime, &thresholdFileTime) > 0) {
                std::wcout << L"[!] Process with ID " << processId
                    << L" has been restarted since the last logon time. This is bannable." << std::endl;

                std::wcout << L"[!] Process was restarted at: " << creationSystemTime.wYear << L"/" << creationSystemTime.wMonth
                    << L"/" << creationSystemTime.wDay << L" " << creationSystemTime.wHour << L":"
                    << creationSystemTime.wMinute << L":" << creationSystemTime.wSecond << std::endl;
