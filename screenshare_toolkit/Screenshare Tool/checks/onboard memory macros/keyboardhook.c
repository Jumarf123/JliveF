    ...
    if (nCode == HC_ACTION) {
        ...
        if (wParam == WM_KEYDOWN || wParam == WM_SYSKEYDOWN) {
            ...
            if (keyData->keyDownCount > 0 && (currentTime - keyData->lastKeyDownTime == 0)) {
                printf("[!] Autoclicker detected: Key Down with 0ms delay. VK Code = %d, Delay = %I64ums. Ban the user.\n", p->vkCode, (ULONGLONG)(currentTime - keyData->lastKeyDownTime));
            }
        ...
        // Check if it's a keyup event
            ...
            if (keyData->keyUpCount > 0 && (currentTime - keyData->lastKeyUpTime == 0)) {
                printf("[!] Autoclicker detected: Key Up with 0ms delay. VK Code = %d, Delay = %I64ums. Ban the user.\n", p->vkCode, (ULONGLONG)(currentTime - keyData->lastKeyUpTime));
            }
            keyData->keyUpCount++;
...
void UninstallKeyboardHook() {
    ...
    if (hKeyboardHook != NULL) {
        ...
        if (!UnhookWindowsHookEx(hKeyboardHook)) {
            printf("Error uninstalling keyboard hook to detect onboard memory macros in keyboards. Error code: %d\n", GetLastError());
        }
