...
const SA_HELPER_SOURCE: &str = include_str!("../sa_helper/ExternalSaDump.java");
...
fn main() {
    if let Err(error) = run_app() {
        display_error_and_wait("External dumper failed", &format!("{error:#}"));
        std::process::exit(1);
    }
}
...
fn run_app() -> Result<()> {
    ...
    let output_path = build_output_path(target.pid)?;
    let helper_files = stage_helper_source(target.pid)?;

    let dump_result = run_sa_dump(target.pid, &runtime.toolchain, &helper_files, &output_path);
...
fn require_admin() -> Result<()> {
    ...
    if result.0 <= 32 {
        return Err(anyhow!(
            "Failed to request elevation via ShellExecuteW, code {}",
            result.0
        ));
    }
    std::process::exit(0);
...
fn list_java_processes() -> Result<Vec<JavaProcessEntry>> {
    ...
    while has_entry {
        ...
        if exe_name.eq_ignore_ascii_case("javaw.exe") {
            let working_set_bytes = process_working_set(entry.th32ProcessID).unwrap_or(0);
...
fn build_output_path(pid: u32) -> Result<PathBuf> {
    ...
    Ok(exe_dir.join(format!("external_classes_{}_{}.txt", pid, timestamp)))
}
